(() => {
  ...
  (function stars() {
    ...
    const loop = () => {
      camera.position.x = Math.sin(performance.now() * 0.0008) * 0.3;
      camera.position.y = Math.cos(performance.now() * 0.0005) * 0.15;
      camera.lookAt(0, 0, 0);
  ...
  (function dial() {
    ...
    if (!section || !arc || !scoreEl) return;

    const ARC_LEN = 377;
    const lerp = (a, b, t) => a + (b - a) * t;
    const scoreFrom = (p) => Math.round(lerp(300, 850, p));
    const colorFor = (score) => (score < 500 ? "#ef4444" : score <= 700 ? "#f59e0b" : "#10b981");
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        arc.style.stroke = colorFor(score);
